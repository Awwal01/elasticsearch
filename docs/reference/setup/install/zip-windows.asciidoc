...
window (recommended not to open terminal as administrator to avoid autoconfiguration error also ensure your internet is on to enable any required download), `cd` to the `%ES_HOME%` directory, for instance:
...
Elastic is easy to clean up, if installation didn't work as expected,
`cd` out of the unzipped folder (`%ES_HOME%`), delete the folder and retry.
